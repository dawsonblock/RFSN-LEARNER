...
async def chat(request: ChatRequest):
    ...
    # Broadcast event
    await broadcast_event(session.session_id, {
        "type": "user_message",
...
async def get_tools():
    ...
    for name, spec in TOOL_REGISTRY.items():
        tools.append({
            ...
            "require_grant": spec.permission.require_explicit_grant,
            "deny_in_replay": spec.permission.deny_in_replay,
            "budget": {
                "calls_per_turn": spec.budget.calls_per_turn,
                "bytes_per_turn": spec.budget.bytes_per_turn,
            },
            "schema": spec.schema,
        })
    return {"tools": tools}
