...
from .arms import Arm, ArmCategory, ARMS_BY_CATEGORY, get_arms_for_category
...
class MultiArmLearner:
    ...
    def select(
        ...
        *,
        ...
        for i, cat in enumerate(cats):
            ...
            stats = [
                ArmStats(arm_key=a, n=n, mean=mu)
                for (a, n, mu) in summary
                if a in arm_keys
            ]
    ...
    def record_rich(
        ...
        *,
        ...
        """
        for cat, arm in selection.arms.items():
            self.db.record_rich(RichOutcome(
                context_key=selection.context_key,
                arm_key=arm.arm_key,
                reward=reward,
                task_id=task_id,
